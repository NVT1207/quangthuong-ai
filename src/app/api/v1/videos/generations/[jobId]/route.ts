// Video job polling endpoint — async upstream (beeknoee/Veo) trả job_id, client poll qua đây.
...
export async function GET(
  ...
  // Build polling URL: {base}/video/generations/{jobId} (cho beeknoee quirk) hoặc {base}/videos/{jobId} (chuẩn OpenAI)
  // Dùng buildEndpointUrl với kind=videos để tận dụng quirk → strip trailing /generations rồi append /{jobId} nếu cần.
  // Cách đơn giản hơn: build base endpoint videos rồi append /{jobId}.
  const videosEndpoint = buildEndpointUrl(upstream.providerType, upstream.baseUrl, "videos");
  const pollUrl = `${videosEndpoint.replace(/\/+$/, "")}/${encodeURIComponent(jobId)}`;
