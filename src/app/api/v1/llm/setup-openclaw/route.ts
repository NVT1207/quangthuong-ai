...
export async function GET(req: NextRequest) {
  ...
  const botToken = (url.searchParams.get("bot_token") || url.searchParams.get("bot") || "").trim();
  const userId = (url.searchParams.get("user_id") || url.searchParams.get("uid") || "").trim();
...
function buildPosix(opts: {
  ...
  return [
    ...
    `mdl = data['agents']['defaults']['model']`,
    `new_primary = slug(HIGH)`,
    `# GIỮ lưới an toàn cũ: primary mới = HIGH, nhưng fallbacks = [medium, small] + (primary cũ + fallbacks cũ).`,
    `# Tránh xoá sạch fallback chain (chiasegpu/openai...) khiến primary lỗi 1 nhịp là bot chết -> lỗi liên tục.`,
    `old_primary = mdl.get('primary')`,
    `old_fb = mdl.get('fallbacks')`,
    `old_fb = old_fb if isinstance(old_fb, list) else []`,
    `mdl['primary'] = new_primary`,
    `fb = []`,
    `for s in [slug(MEDIUM), slug(SMALL), old_primary] + old_fb:`,
    `    if isinstance(s, str) and s and s != new_primary and s not in fb:`,
    `        fb.append(s)`,
    `mdl['fallbacks'] = fb`,
    ``,
    `# ---- gateway: scaffold port + auth token nếu CHƯA có (config mới) ----`,
    `# Tránh user cài lần đầu nhận config gateway-less -> daemon không boot -> "lỗi liên tục".`,
    `# setdefault: KHÔNG đè giá trị cũ nếu user đã có gateway.`,
    `gw = data.setdefault('gateway', {})`,
    `if isinstance(gw, dict):`,
    `    gw.setdefault('mode', 'local')`,
    `    gw.setdefault('port', 18789)`,
    `    gw.setdefault('bind', '127.0.0.1')`,
    `    if not isinstance(gw.get('auth'), dict):`,
    `        gw['auth'] = {}`,
    `    if not gw['auth'].get('token'):`,
    `        gw['auth']['token'] = secrets.token_hex(16)`,
    ``,
...
function buildPowerShell(opts: {
  ...
  return [
    ...
    `}`,
    `if (-not $PythonCmd) {`,
    `  Write-Host "  [!] Cần cài Python trước rồi chạy lại. Tải tại: https://www.python.org/downloads/windows/" -ForegroundColor Yellow`,
    `  exit 1`,
    `}`,
    ``,
    `$env:BASE_URL = $BaseUrl`,
    `$env:API_KEY = $ApiKey`,
    `$env:SMALL_MODEL = $Small`,
    `$env:MEDIUM_MODEL = $Medium`,
    `$env:HIGH_MODEL = $High`,
    `$env:BOT_TOKEN = $BotToken`,
    `$env:TG_USER_ID = $TgUserId`,
